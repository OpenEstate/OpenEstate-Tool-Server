...
# launch a live backup on the database server
...
BACKUP_DIR="./var/backup/$MANAGER_URLID/"
...
exec "$JAVACMD" \
  ...
  -classpath "./etc:./lib/*" \
  $JAVA_OPTS \
  org.hsqldb.cmdline.SqlTool --rcFile="$MANAGER_CONF" --sql="
    CHECKPOINT;
    BACKUP DATABASE TO '$BACKUP_DIR' BLOCKING;
  " $MANAGER_URLID
