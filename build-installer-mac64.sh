...
TEMP_DIR="$TARGET_DIR/mac64-build"
...
for f in ${PACKAGE_DIR}/*.mac64.tar.gz; do
    ...
    FOUND="1"
    pkgName=$(basename ${f%.mac64.tar.gz})
    ...
    echo ""
    echo "----------------------------------------------------------------"
    echo "Creating $pkgName.dmg..."
    echo "----------------------------------------------------------------"
    echo ""
    rm -f "$PACKAGE_DIR/$pkgName.dmg"
    dmgbuild \
        -s "$SRC_DIR/settings.py" \
        -D app="$TEMP_DIR/$pkg" \
        "$appName Installer" \
        "$PACKAGE_DIR/$pkgName.dmg"

    echo ""
    echo "----------------------------------------------------------------"
    echo "Signing $pkgName.dmg..."
    echo "----------------------------------------------------------------"
    echo ""
    codesign --force --sign "$KEY" "$PACKAGE_DIR/$pkgName.dmg"
    echo "Verifying signature:"
    codesign -d --verbose=4 "$PACKAGE_DIR/$pkgName.dmg"
    echo ""
    echo "Verifying access for Gatekeeper:"
    spctl -a -t open --context context:primary-signature -v "$PACKAGE_DIR/$pkgName.dmg"
